...
const App: React.FC = () => {
  ...
  const handleArchiveRetrieval = async () => {
    ...
    if (result) {
      ...
      alert("Could not find this hymn in the sacred archive. Please check the number.");
    }
  };
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden font-sans">
        ...
        <div className="flex flex-col h-full">
          <div className="p-6 border-b border-gray-50 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-gray-900 tracking-tight leading-none mb-1">IOM Hymnal</h1>
                <p className="text-[10px] text-gray-400 uppercase font-black tracking-widest">Archive Edition</p>
              </div>
            </div>
          ...
          <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
            <p className="px-4 mb-2 text-xs font-bold text-gray-400 uppercase tracking-widest">Navigation</p>
            <SidebarLink 
              icon={<Library size={20} />} 
              label="The Full 858" 
              active={currentView === 'list' && selectedCategory === 'All'} 
              onClick={() => { setCurrentView('list'); setSelectedCategory('All'); setShowFavoritesOnly(false); closeSidebar(); }}
            />
            <SidebarLink 
              icon={<Heart size={20} />} 
              label="Saved Verses" 
              active={currentView === 'favorites'} 
              onClick={() => { setCurrentView('favorites'); setShowFavoritesOnly(false); closeSidebar(); }}
            />
            <SidebarLink 
              icon={<Grid size={20} />} 
              label="Liturgy Index" 
              active={currentView === 'categories'} 
              onClick={() => { setCurrentView('categories'); setShowFavoritesOnly(false); closeSidebar(); }}
            />

            <div className="pt-6">
              <p className="px-4 mb-2 text-xs font-bold text-gray-400 uppercase tracking-widest">Sacred Groups</p>
              <div className="grid grid-cols-1 gap-1 px-2">
                {CATEGORIES.slice(1, 10).map(cat => (
                  <button
                    key={cat}
                    onClick={() => { setSelectedCategory(cat); setCurrentView('list'); setShowFavoritesOnly(false); closeSidebar(); }}
          ...
          <div className="p-4 border-t border-gray-50">
            <div className={`rounded-3xl p-5 text-white relative overflow-hidden group transition-all ${isOffline ? 'bg-gray-400' : 'bg-gradient-to-br from-indigo-950 to-indigo-800'}`}>
              <div className="relative z-10">
                <h4 className="font-black text-[10px] mb-1 uppercase tracking-widest text-indigo-300">Archive Retrieval</h4>
                <p className="text-[11px] text-indigo-100/80 mb-3 leading-tight">
                  {isOffline ? 'Archive unavailable while offline.' : 'Access all 600+ hymns instantly via search.'}
                </p>
                <div className="flex items-center gap-2">
                  <div className={`w-1.5 h-1.5 rounded-full ${isOffline ? 'bg-gray-300' : 'bg-green-400 animate-pulse'}`}></div>
                  <span className={`text-[9px] font-bold uppercase ${isOffline ? 'text-gray-200' : 'text-green-400'}`}>
      ...
      <main className="flex-1 flex flex-col h-full relative">
        {currentView === 'detail' && selectedHymn ? (
          ...
          <>
            ...
            <header className="bg-white border-b border-gray-100 px-6 py-5 flex flex-col gap-5 sticky top-0 z-30 shadow-sm">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <button onClick={() => setIsSidebarOpen(true)} className="p-2 lg:hidden text-gray-600 hover:bg-gray-100 rounded-xl">
                    <Menu size={24} />
                  </button>
                  <div>
                    <h2 className="text-2xl font-black text-gray-900 tracking-tight leading-none">
                      {currentView === 'favorites' || showFavoritesOnly ? (
                        selectedCategory === 'All' ? 'Saved Verses' : `Saved in ${selectedCategory}`
                      ) : (
                        selectedCategory === 'All' ? 'Sacred Hymnal' : selectedCategory
                      )}
                    </h2>
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-[0.2em] mt-1.5">Full English 858+ Edition</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <button 
                  ...
                  >
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">{showFavoritesOnly ? 'Favorites On' : 'All Hymns'}</span>
                  </button>
                  <div className="bg-indigo-50 px-4 py-2 rounded-2xl border border-indigo-100 flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-black text-indigo-700 uppercase tracking-widest">{filteredHymns.length} Local</span>
                  </div>
                  {isOffline && (
              ...
              <div className="relative group flex gap-2">
                ...
                {searchQuery && filteredHymns.length === 0 && (
                  ...
                  >
                    {isSearchingArchive ? <Loader2 size={16} className="animate-spin" /> : <Sparkles size={16} />}
                    {isSearchingArchive ? 'Archiving...' : isOffline ? 'Offline' : 'Get from Archive'}
                  </button>
                )}
              </div>
            </header>
            ...
            <div className="flex-1 overflow-y-auto pb-10 bg-gray-50/50">
               {currentView === 'categories' ? (
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6">
                    {CATEGORIES.map(cat => (
                      <button
                        key={cat}
                        onClick={() => { setSelectedCategory(cat); setCurrentView('list'); }}
                      ...
                      >
                        ...
                        <span className="font-black text-gray-700 text-[10px] text-center uppercase tracking-widest">{cat}</span>
                      </button>
                    ))}
                 </div>
               ) : (
               ...
               {filteredHymns.length === 0 && !isSearchingArchive && searchQuery && (
                 <div className="flex flex-col items-center justify-center py-20 px-10 text-center space-y-6">
                    ...
                    <div className="max-w-xs">
                      <h4 className="text-xl font-black text-gray-900 mb-2 tracking-tight">Not in Local Core</h4>
        ...
        {(organPlayer.isPlaying || organPlayer.isLoadingAudio) && currentView !== 'detail' && (
...
const SidebarLink: React.FC<SidebarLinkProps> = ({ icon, label, active, onClick }) => (
  <button 
    ...
    `}
  >
    <span className={`${active ? 'text-indigo-300' : 'text-gray-300'}`}>{icon}</span>
    {label}
  </button>
);
